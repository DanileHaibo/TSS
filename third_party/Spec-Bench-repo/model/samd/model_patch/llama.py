...
try:
    ...
    from transformers.models.llama.modeling_llama import LlamaForCausalLM, LlamaModel, logger
...
def _uses_sdpa(model: LlamaModel) -> bool:
    impl = getattr(model.config, "_attn_implementation", None)
    if impl is not None:
        return impl == "sdpa"
    return bool(getattr(model, "_use_sdpa", False))


def _uses_flash_attention_2(model: LlamaModel) -> bool:
    impl = getattr(model.config, "_attn_implementation", None)
    if impl is not None:
        return impl == "flash_attention_2"
    return bool(getattr(model, "_use_flash_attention_2", False))
...
def llama_model_forward(
    ...
    if use_cache:
        ...
        if transformers.__version__ >= "4.43":
            get_len = getattr(past_key_values, "get_usable_length", None)
            past_key_values_length = get_len(seq_length) if callable(get_len) else past_key_values.get_seq_length()
        else:
    ...
    if _uses_flash_attention_2(self):
        raise NotImplementedError
        # 2d mask is passed through the layers
        attention_mask = attention_mask if (attention_mask is not None and 0 in attention_mask) else None
    elif _uses_sdpa(self) and not output_attentions:
    ...
    # transformers >=4.57 uses past_key_values; older SAMD paths used past_key_value.
